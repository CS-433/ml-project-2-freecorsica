...
def init_config():
    ...
    parser.add_argument('--cand', type=int, default=1, help='number of candidate')
...
class AugumentedDataset(TensorDataset):
    def __init__(self, mode, engine):
        ...
        self.engine = engine
        self.data = {}
        with open(f'saved_datasets/persona_chat_peacok/{mode}_dataset.pkl', 'rb') as f:
            self.data['persona_chat_peacok'] = pickle.load(f)
...
def main():
    ...
    args.output_dir = f"{'test2'}{data_from}"
    ...
    if args.load_from != None:
        ...
        model = BartForConditionalGeneration.from_pretrained(
            "facebook/bart-base", num_labels=1
        )
