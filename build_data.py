...
def create_data_from_peacok_format(
    ...
    if filename is not None:
        ...
        inferred_filename = {
            ...
            'persona_revised_chat': '{}_persona_revised_chat_ext.json'
            }
        filepath = os.path.join(data_dir, inferred_filename[dataset].format(mode))
        print('FILEPATH: ', filepath)

    augumented_info_path = os.path.join(data_dir, "persona_extend_full_original.json")

    data_dict = json.load(open(filepath))
    augumented_info = json.load(open(augumented_info_path))
